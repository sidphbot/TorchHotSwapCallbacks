"""Scenario: plateau_stagnation — Loss flattens after initial decay, triggering stagnation detection."""
import os
import random
import threading
import time
...
def train_fn(
    ...
    for step in range(1, max_steps + 1):
        ...
        if step <= 50:
            # Normal decay phase: loss drops from 2.0 toward ~1.0
            decay = 0.02 + random.gauss(0, 0.003)
            loss = loss * (1 - decay)
        elif step <= 80:
            # Plateau phase: loss stagnates with tiny noise (range < 0.002)
            if plateau_value is None:
                plateau_value = loss
            loss = plateau_value + random.uniform(-0.0005, 0.0005)
        else:
            # Post-plateau: slow recovery (rule may have acted)
            loss = loss * (1 - 0.005) + random.gauss(0, 0.002)
