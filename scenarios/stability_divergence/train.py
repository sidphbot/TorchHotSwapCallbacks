"""Scenario: stability_divergence — Loss divergence triggers recovery."""
import os
import random
import threading
import time
...
def train_fn(
    ...
    for step in range(1, max_steps + 1):
        ...
        grad_norm = 1.5 + random.uniform(-0.5, 1.0)

        if step <= 35:
            # Normal decay: ~2% per step
            loss = loss * (1 - 0.02) + random.gauss(0, 0.005)
        elif 36 <= step <= 40:
            # Divergence: loss increases by ~0.1-0.15 per step over 5 steps
            # Total increase > 0.5 over the window, triggering loss_spike_recovery
            loss = loss + 0.10 + random.uniform(0.02, 0.05)
            grad_norm = 5.0 + random.uniform(0, 3.0)  # elevated but not spiking
        else:
            # Post-divergence: resume decay (possibly with reduced LR from rule)
            loss = loss * (1 - 0.015) + random.gauss(0, 0.005)
