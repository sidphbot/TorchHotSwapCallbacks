"""Scenario: distill_warmup — Early warmup triggers distill_weight boost."""
import os
import random
import threading
import time
...
def train_fn(
    ...
    for step in range(1, max_steps + 1):
        ...
        # Slow, steady decay — stays in early warmup territory (step < 200)
        # for most of the run, which should trigger summary_first_warmup
        decay = 0.005 + random.gauss(0, 0.002)
        loss = max(0.3, loss * (1 - decay) + random.gauss(0, 0.01))
        distill_loss_val = max(0.5, distill_loss_val * (1 - decay * 0.8) + random.gauss(0, 0.02))
